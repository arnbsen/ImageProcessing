...
int main(){
    ...
    FILE *fp1,*prs,*outf,*mode;
    int *arr;
    int outr[524288],out[524288];
    int i = 0,j=0,k=0,l,p,r;
    int store;
    int con=32700;
    ...
    fp1=fopen("/Users/arnabsen/Desktop/FileHandling/abc1.txt", "r+");
    prs=fopen("/Users/arnabsen/Desktop/FileHandling/prs.txt", "w+");
    outf=fopen("/Users/arnabsen/Desktop/FileHandling/outfile5.txt", "w+");
    
    while (fscanf(fp1, "%d",&store)&&!feof(fp1)) {
        i++;
    }
    arr = (int*)malloc((i+3)*sizeof(int));
    if (arr==NULL) {
 ...
 /*   while (fgets(swr, 100, fp1)&&!feof(fp1)) {
    ...
    while (fscanf(fp1, "%d",&temp)&&!feof(fp1)) {
        if (temp==-1) {
            fprintf(outf, "\n");
